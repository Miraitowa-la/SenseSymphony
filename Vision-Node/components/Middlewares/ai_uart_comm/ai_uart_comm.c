...
esp_err_t ai_uart_init(const ai_uart_config_t *config)
{
    ...
    ESP_ERROR_CHECK(uart_driver_install(s_uart_port, 2048, 0, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(s_uart_port, &uart_config));
    ESP_ERROR_CHECK(uart_set_pin(s_uart_port, config->tx_gpio, config->rx_gpio,
...
{
    return mode < AI_UART_MODE_COUNT ? s_mode_names[mode] : "UNKNOWN";
}
...
{
    static ai_uart_snapshot_t pending;
    ...
    unsigned y10;
    int action;
    unsigned detect_confidence;
    unsigned action_confidence;

    if (!snapshot || !read_line(line, sizeof(line))) {
        return false;
    }

    if (sscanf(line, "R,%lu,%d,%u", &seq, &mode, &count) == 3) {
        pending.sequence = (uint32_t)seq;
        pending.mode = mode >= 0 && mode < AI_UART_MODE_COUNT ? (ai_uart_mode_t)mode : AI_UART_MODE_FACE;
        pending.object_count = 0;
        expected_count = count > AI_UART_MAX_OBJECTS ? AI_UART_MAX_OBJECTS : count;
        if (expected_count == 0) {
            *snapshot = pending;
            return true;
        }
    ...
    if (sscanf(line, "O,%u,%u,%d,%u,%u", &x10, &y10, &action, &detect_confidence, &action_confidence) == 5) {
        if (pending.object_count < expected_count) {
            ai_uart_object_t *object = &pending.objects[pending.object_count++];
            object->x10 = x10 > 1000 ? 1000 : (uint16_t)x10;
            object->y10 = y10 > 1000 ? 1000 : (uint16_t)y10;
            object->action = (int16_t)action;
            object->detect_confidence = detect_confidence > 1000 ? 1000 : (uint16_t)detect_confidence;
            object->action_confidence = action_confidence > 1000 ? 1000 : (uint16_t)action_confidence;
        }
        if (pending.object_count >= expected_count) {
            *snapshot = pending;
            return true;
